...
def parse_args():
    ...
    parser.add_argument('--synapse_depth', type=int, default=4,
                        help='Depth of U-NET model for synapse. 1=linear, no unet.')
    ...
    parser.add_argument('--n_synch_action', type=int, default=32,
                        help='Number of neurons to use for observation/action synch.')
    ...
    parser.add_argument('--dropout_nlm', type=float, default=None,
                        help='Dropout rate for NLMs specifically. Unset to match dropout on the rest of the model.')
...
if __name__ == '__main__':
    ...
    testloader = torch.utils.data.DataLoader(test_data, batch_size=args.batch_size_test, shuffle=True, num_workers=1,
                                             drop_last=False)
    ...
    with tqdm(total=args.training_iterations, initial=start_iter, leave=False, position=0, dynamic_ncols=True) as pbar:
        for bi in range(start_iter, args.training_iterations):
            ...
            pbar.set_description(
                f'Sorting {args.N_to_sort} real numbers. Loss={loss.item():0.3f}. Accuracy={accuracy:0.3f}. LR={current_lr:0.6f}')
                    ...
                    with torch.inference_mode():
                        ...
                        with tqdm(total=len(loader), initial=0, leave=False, position=1,
                            ...
                            for inferi, (inputs, targets) in enumerate(loader):
                                ...
                                decoded = [d[:targets.shape[1]] for d in
                                           decode_predictions(these_predictions, predictions.shape[1] - 1)]
                                decoded = torch.stack([torch.concatenate((d, torch.zeros(targets.shape[1] - len(d),
                                                                                         device=targets.device) +
                                                                          targets.shape[1])) if len(d) < targets.shape[
                                    1] else d for d in decoded], 0)
