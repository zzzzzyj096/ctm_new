...
from models.modules import SuperLinear, LearnableFourierPositionalEncoding, MultiLearnableFourierPositionalEncoding, CustomRotationalEmbedding, CustomRotationalEmbedding1D
...
class SuperLinear(nn.Module):
    """健壮的 SuperLinear 层：实现神经元级模型 (NLMs)"""
    def __init__(self, in_dims, out_dims, N, dropout=0.0):
        ...
        # 权重形状: [neurons, in_dims, out_dims]
        self.weight = nn.Parameter(
            torch.empty(N, in_dims, out_dims).uniform_(
                -1/math.sqrt(in_dims + out_dims),
                1/math.sqrt(in_dims + out_dims)
            )
        )

        # 偏置: [1, neurons, out_dims]
        self.bias = nn.Parameter(torch.zeros(1, N, out_dims))
    ...
    def forward(self, x):
        """
        参数:
            x: 输入张量，形状为 (batch, neurons, features) 或 (batch, features)
        返回:
            输出张量，形状为 (batch, neurons, out_dims)
        """
        # 处理不同维度的输入
        if x.dim() == 2:
            # 输入为 (batch, features) -> 添加神经元维度
            x = x.unsqueeze(1)  # 变为 (batch, 1, features)
        elif x.dim() != 3:
            raise ValueError(f"输入应为2D或3D张量，但得到 {x.dim()}D")
        batch_size, num_neurons, num_features = x.size()

        # 确保神经元维度匹配
        if num_neurons != self.N:
          # 计算需要重复的次数
          repeat_count = (self.N + num_neurons - 1) // num_neurons

          # 沿神经元维度重复
          x = x.repeat_interleave(repeat_count, dim=1)
        ...
        # 使用更直观的 einsum 方程
        # 输入: [batch, neurons, in_dims]
        # 权重: [neurons, in_dims, out_dims]
        # 输出: [batch, neurons, out_dims]
        out = torch.einsum('bni,nio->bno', x, self.weight) + self.bias
...
class ContinuousThoughtMachineSIMPLE(nn.Module):
    def __init__(self,
                 ...
                 exc_inh_ratio=0.7  #补充
                 ):
        super(ContinuousThoughtMachineSIMPLE,self).__init__()

         # 激活/抑制神经元划分
        self.exc_inh_ratio = exc_inh_ratio
        self.n_exc = int(d_model * exc_inh_ratio)  # 激活神经元数量
        self.n_inh = d_model - self.n_exc
        # --- Core Parameters ---
        ...
        self.synapses = nn.Sequential(
                nn.Dropout(dropout),
                nn.LazyLinear(d_model * 4),  #之前是2，修改为4
                nn.GLU(),
                # 激活/抑制独立处理
                SuperLinear(in_dims=d_model*2, out_dims=self.n_exc, N=self.n_exc),
                nn.ReLU(),
                SuperLinear(in_dims=self.n_exc, out_dims=self.n_inh, N=self.n_inh),
                nn.ReLU(),
                Negative(),
                # 添加一个线性层将输出维度调整为 d_model
                nn.Flatten(start_dim=1),
                nn.Linear(self.n_inh * self.n_inh, d_model),
                nn.LayerNorm(d_model)
            )


        # 分别定义激活和抑制神经元的轨迹处理器
        self.trace_processor_exc = nn.Sequential(
            SuperLinear(in_dims=memory_length, out_dims=memory_hidden_dims, N=self.n_exc),
            nn.ReLU(),
            SuperLinear(in_dims=memory_hidden_dims, out_dims=1, N=self.n_exc)
        )

        self.trace_processor_inh = nn.Sequential(
            SuperLinear(in_dims=memory_length, out_dims=memory_hidden_dims, N=self.n_inh),
            nn.ReLU(),
            SuperLinear(in_dims=memory_hidden_dims, out_dims=1, N=self.n_inh)
        )
    ...
    def forward(self, x, track=False):
        ...
        for stepi in range(self.iterations):
            ...
            # 分别处理激活和抑制神经元的轨迹
            exc_state = self.trace_processor_exc(state_trace[:, :self.n_exc, :])
            inh_state = self.trace_processor_inh(state_trace[:, self.n_exc:, :])
            # 确保状态是二维的
            exc_state = exc_state.squeeze(-1)
            inh_state = inh_state.squeeze(-1)
            activated_state = torch.cat([exc_state, inh_state], dim=1).squeeze(-1)
